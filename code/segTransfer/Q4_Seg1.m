...
[boundingBoxes] = batchDetectImageESVM(gtImages_left, models, params);

%% Cleaning up the bounding boxes
cleanedBoxes=[];

for i=1:length(gtImages_left)
    ...
    I=imread([source,gtImages_left{i}]);
%     figure;
%     showboxes(I,box);
...
load('Im_leftsegment.mat','mask','maskedImage');

for i=1:length(gtImages_left)
    thismodel=models{93};
    img=imread([source,gtImages_left{i}]);
    box=cleanedBoxes(i,:);
